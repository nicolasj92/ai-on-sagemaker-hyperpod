...
<TabItem value="ml.m5.2xlarge" label="ml.m5.2xlarge (Workshop Studio)">

:::note
For Workshop Studio accounts, we'll use an M5 general purpose instance for demonstration purposes.
:::
...
:::warning Important
Before creating your HyperPod cluster, consider the types of experiments you intend to run. For example, if you want to run the [Fully Sharded Data Parallel(FSDP)](/docs/01-eks-blueprints/training/fsdp/fully-sharded-data-parallel.md) experiment, it has been verified on a cluster of 8 `ml.g5.8xlarge` nodes, whereas the [Llama 3 model training using Trainium](/docs/01-eks-blueprints/training/trainium/aws-trainium.md) experiment has been verified on 1 to 2 node clusters of `ml.trn1.32xlarge` or `ml.trn1n.32xlarge` instances. To choose a different accelerated instance type and count, refer back to the [Setup Environment Variables](/docs/00-getting-started/orchestrated-by-eks/Verifying%20cluster%20connection%20to%20EKS.md) section.
:::
...
<TabItem value="simplified" label="Simplified (Workshop Studio)">
