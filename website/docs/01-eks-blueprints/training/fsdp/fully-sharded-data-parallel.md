...
Before running this training, you'll need to create a cluster with Amazon EKS on SageMaker HyperPod. 

Instructions can be found in the [0. Deploy Your HyperPod Cluster](/docs/category/getting-started) section. Please follow them if you haven't done so already.

Please also make sure that you deployed GPU device plugin, EFA device plugin, and Kubeflow training operator to your cluster. 

See [What Dependencies are Installed on Your EKS Cluster](/docs/category/getting-started) for details.

To build a container image, you need a x86-64 based development environment with Docker installed. If you use recent Mac with Apple Silicon, they are not x86-64 based but ARM based. You can either SageMaker Code Editor for this purpose or make sure you build the container for a x86_64 platform (instructions on that later).

### Verified instance types, instance counts

- ml.g5.8xlarge x 8
- ml.p5en.48xlarge x 2

## Setup the docker image 

### Build a docker image and dataset
...
1. Build the `fsdp` image.
    ...
    **Alternatively**, ff you are on a Mac, use `buildx` to target `linux/amd64` architecture:  
    ...
    ```
    Building image can take 5~7min. If successful, you should see following success message at the end.
...
When you run for the first time, it takes 3~4min until the Pod statuses change from `ContainerCreating` to `Running`.
